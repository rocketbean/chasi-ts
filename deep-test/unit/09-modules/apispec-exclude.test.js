...
describe("isDriverAllowed", () => {
    ...
    it("honors exclude", () => {
        expect(SchemaCollection.isDriverAllowed("test", { drivers: { exclude: ["test"] } })).toBe(false);
        expect(SchemaCollection.isDriverAllowed("dev", { drivers: { exclude: ["test"] } })).toBe(true);
    });
    it("lets include win over exclude", () => {
        const filter = { drivers: { include: ["dev"], exclude: ["test"] } };
...
describe("isModelAllowed", () => {
    ...
    it("excludes case-insensitively", () => {
        const filter = { models: { dev: { exclude: ["User"] } } };
    ...
    it("include wins over exclude", () => {
        const filter = { models: { dev: { include: ["Post"], exclude: ["post"] } } };
        expect(SchemaCollection.isModelAllowed("dev", "post", filter)).toBe(true);
        expect(SchemaCollection.isModelAllowed("dev", "user", filter)).toBe(false);
    });
});
describe("collection-level exclude", () => {
    ...
    it("skips an excluded model during collection", () => {
        new SchemaCollection().collectSchemas({ models: { dev: { exclude: ["secret"] } } });
