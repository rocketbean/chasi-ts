...
beforeEach(() => {
    ...
    globalThis.$app = { $modules: { RouterModule: { routers: [api, admin] } } };
});
afterEach(() => {
...
describe("SdkBuilder RouteCollector.init", () => {
    it("collects only the configured routers and skips excluded routes", async () => {
        await new RouteCollector().init({ routers: ["api"], exclude: [{ m: "post", url: "/api/users/forget" }] });
    ...
    it("marks protected vs public and derives namespace/function names", async () => {
        await new RouteCollector().init({ routers: ["api"], exclude: [] });
