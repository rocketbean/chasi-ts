...
function make(cfg: any = { enabled: true, engines: [] }) {
...
describe("CompilerEngine", () => {
  ...
  it("setInvoker stores the app reference", () => {
    ...
    expect((c as any).$app).toEqual({ marker: true });
  });
  ...
  it("mount is a no-op when the engine is disabled", async () => {
    ...
    await expect(c.mount({ property: { prefix: "/" } } as any, ["web"])).resolves.toBeUndefined();
  });
});
