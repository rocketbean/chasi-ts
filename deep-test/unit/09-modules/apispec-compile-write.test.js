...
beforeEach(() => {
    ...
    globalThis.$app = { $modules: { Database: { $databases: {} }, RouterModule: { routers: [] } } };
});
afterEach(() => {
...
describe("Compiler.compile", () => {
    it("assembles an OpenAPI document from config + collections", () => {
        ...
        RouterCollection.paths = { "/api/x": { get: {} } };
        const schemas = new SchemaCollection();
        const routers = new RouterCollection();
        const doc = new Compiler(baseConfig("ignore.json"), schemas, routers).compile();
