...
describe("Model.connect (mongodb)", () => {
    it("returns connection.model(name, schema) for a mongodb connection", () => {
        const model = vi.fn((n) => ({ modelName: n }));
        Models.$databases = { c: { config: { driver: "mongodb" }, connection: { model } } };
    ...
    it("defaults to the '_' connection", () => {
        const model = vi.fn((n) => ({ modelName: n }));
        Models.$databases = { _: { config: { driver: "mongodb" }, connection: { model } } };
        Model.connect("User", {});
        expect(model).toHaveBeenCalled();
    });
    it("returns undefined for a non-mongodb driver", () => {
        Models.$databases = { c: { config: { driver: "drizzle" }, connection: {} } };
        expect(Model.connect("User", {}, "c")).toBeUndefined();
    });
    it("returns undefined (and does not throw) for a missing connection", () => {
        expect(Model.connect("User", {}, "ghost")).toBeUndefined();
    });
});
describe("Model.drizzle", () => {
    it("returns the live client for a drizzle connection", () => {
        Models.$databases = { d: { driverName: "drizzle", connection: { q: 1 } } };
...
describe("Model.prisma", () => {
    ...
    it("throws for a missing or mismatched connection", () => {
        ...
        Models.$databases = { p: { driverName: "mongodb", connection: {} } };
