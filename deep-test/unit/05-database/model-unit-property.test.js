...
describe("Unit model", () => {
    it("is named 'unit' with name/code/property paths", () => {
        ...
        expect(Object.keys(Unit.schema.paths)).toEqual(expect.arrayContaining(["name", "code", "property"]));
    });
    it("requires name, code and property", async () => {
        const err = await new Unit({}).validate().catch((e) => e);
...
describe("Property model", () => {
    it("is named 'property' with name/code/units paths", () => {
        ...
        expect(Object.keys(Property.schema.paths)).toEqual(expect.arrayContaining(["name", "code", "units"]));
    });
    it("requires name and code (units is an optional array)", async () => {
        const err = await new Property({}).validate().catch((e) => e);
        expect(err.errors.name).toBeDefined();
        expect(err.errors.code).toBeDefined();
