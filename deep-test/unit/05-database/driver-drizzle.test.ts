...
function makeDriver(options: any) {
  return new DrizzleDriver({ url: "postgres://u:p@host/db", options, hideLogConnectionStrings: false } as any, "pg");
}
...
describe("DrizzleDriver", () => {
  ...
  it("connect forwards the url as `connection` and loads the schema", async () => {
    ...
    let captured: any;
    (d as any).loadSchema = async () => ({ users: { id: 1 } });
    (d as any).getDrizzleFn = async () => (cfg: any) => ((captured = cfg), { __db: true });
    const stop = vi.fn();
