...
beforeAll(async () => {
    ...
    Authorization.$drivers = { _: { property: { key: "unit-secret" } } };
...
describe("user model", () => {
    ...
    it("requires name, alias and email", async () => {
        const err = await new User({}).validate().catch((e) => e);
    ...
    it("rejects a password containing the word 'password'", async () => {
        const err = await new User({ name: "a", alias: "al", email: "a@b.co", password: "superpassword" })
            .validate()
            .catch((e) => e);
        expect(err).toBeTruthy();
    });
    ...
    it("generateAuthToken issues a verifiable JWT signed with the driver key", async () => {
        ...
        const decoded = jwt.verify(token, "unit-secret");
