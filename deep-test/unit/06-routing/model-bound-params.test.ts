...
describe("model-bound params", () => {
  it("delete() calls delete() on the bound __user document", async () => {
    ...
    const out = await makeUC().delete({ params: { __user } } as any, {} as any);
    expect(__user.delete).toHaveBeenCalledOnce();
    expect(out).toBe(true);
  });

  it("update() merges the body into the bound __user and saves it", async () => {
    const __user: any = { name: "old", save: vi.fn(async function (this: any) { return this; }) };
    const out: any = await makeUC().update({ params: { __user }, body: { name: "new" } } as any, {} as any);
