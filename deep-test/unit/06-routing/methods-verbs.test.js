...
function makeRoute() {
    return new Route(new Registry({ prefix: "x", auth: null, AuthRouteExceptions: [] }));
}
describe("Methods registry", () => {
    it("exports a marker class for every supported verb", () => {
        expect(Object.keys(methods)).toEqual(expect.arrayContaining(["Get", "Post", "Options", "Patch", "Put", "Search", "Delete"]));
    });
});
describe("Route verbs", () => {
    it.each(["get", "post", "put", "patch", "delete", "options", "search"])("route.%s registers an endpoint with that method", (verb) => {
