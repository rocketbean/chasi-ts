...
beforeEach(() => {
  ...
  Router.Controllers = { "ctrls/usercontroller": { instance: { list: () => {} } } } as any;
  Router.Middlewares = { auth: (_req: any, _res: any, next: any) => next() } as any;
});

describe("group middleware", () => {
  it("applies a group's middleware alias to every child endpoint", async () => {
    const registry = new Registry({ prefix: "users", auth: null, AuthRouteExceptions: [] } as any);
