...
describe("NetServer registry", () => {
  ...
  it("wires the observer onto SocketRouter without a pipe", () => {
    const observer: any = { tag: "obs" };
    new NetServer(undefined, observer);
    expect((SocketRouter as any).$observer).toBe(observer);
  });
});
