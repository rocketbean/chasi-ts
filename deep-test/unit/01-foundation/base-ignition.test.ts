...
describe("Base › mergeObjects", () => {
  ...
  it("merges shallow keys", () => {
    expect(Base.mergeObjects({ a: 1 } as any, { b: 2 } as any)).toEqual({ a: 1, b: 2 });
  });

  it("deep-merges nested structures", () => {
    const merged = Base.mergeObjects({ a: { x: 1 } } as any, { a: { y: 2 } } as any);
    expect(merged).toEqual({ a: { x: 1, y: 2 } });
  });

  it("mutates and returns the target", () => {
    const target: any = { a: 1 };
    const result = Base.mergeObjects(target, { b: 2 } as any);
...
describe("Base › ProxyHandler", () => {
  it("returns plain values for accessible keys", () => {
    const proxied = new Proxy({ name: "chasi", count: 3 } as any, ProxyHandler as any);
...
describe("Base › Ignition", () => {
  ...
  it("loads the config directory into a registry object", async () => {
    const registry: any = await Base.Ignition();
    ...
    expect(Object.keys(registry)).toEqual(
      expect.arrayContaining(["server", "database", "authentication"]),
    );
  });
});
