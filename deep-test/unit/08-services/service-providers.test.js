...
describe("ApiSpecServiceProvider.beforeServerBoot", () => {
    it("is a no-op when apispec is disabled", async () => {
        Provider.config = { apispec: { enabled: false } };
        await new ApiSpecServiceProvider().beforeServerBoot();
        expect(ApiSpec.init).not.toHaveBeenCalled();
    });
    it("initializes and compiles the spec when enabled", async () => {
        const cfg = { enabled: true, output: { filename: "api.spec.json" } };
        Provider.config = { apispec: cfg };
        await new ApiSpecServiceProvider().beforeServerBoot();
        expect(ApiSpec.init).toHaveBeenCalledWith(cfg);
        expect(ApiSpec.instance.compile).toHaveBeenCalledOnce();
    });
});
describe("SdkBuilderServiceProvider.beforeServerBoot", () => {
    it("is a no-op when sdkbuilder is disabled", async () => {
        Provider.config = { sdkbuilder: { enabled: false } };
        await new SdkBuilderServiceProvider().beforeServerBoot();
        expect(SdkBuilder.init).not.toHaveBeenCalled();
    });
    it("builds and clears handlers when enabled", async () => {
        const cfg = { enabled: true };
        Provider.config = { sdkbuilder: cfg };
        await new SdkBuilderServiceProvider().beforeServerBoot();
        expect(SdkBuilder.init).toHaveBeenCalledWith(cfg);
        expect(SdkBuilder.instance.compile).toHaveBeenCalledOnce();
        expect(SdkBuilder.instance.clearSdkHandlers).toHaveBeenCalledOnce();
    });
});
