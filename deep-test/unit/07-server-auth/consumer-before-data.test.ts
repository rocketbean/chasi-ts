...
function makeEndpoint(method = "get", path = "/ping") {
  const ep = new Endpoint({ method, controller: () => {}, endpoint: path.slice(1), options: {} } as any, []);
  ep.path = path;
  ep.useAuth = (_q: any, _s: any, n: any) => n();
  ep.$middlewares = [];
  ep.isDynamic = false;
  ep.beforeFns = [];
  ep.afterFns = [];
...
describe("Consumer.consume › happy path", () => {
  it("runs before/after hooks, exposes data(), and serializes the return value", async () => {
    const consumer = new Consumer({} as any);
    const before = vi.fn();
    const after = vi.fn();
    const ep = makeEndpoint();
    ep.$controller = {} as any;
    ep.$method = async (_req: any, _res: any, data: any) => ({ pong: true, injected: data() });
    ep.beforeFns = [before];
    ep.afterFns = [after];
    const router: any = { $registry: { routes: [ep] }, property: { data: () => ({ tenant: "acme" }) } };
...
describe("Consumer.bindModel", () => {
  it("binds a known model param to its document and nulls unknown ones", async () => {
    ...
    Models.collection = { user: { findById: vi.fn(async (id: string) => ({ _id: id })) } } as any;
    const params: any = { user: "42", widget: "9" };
