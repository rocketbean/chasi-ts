...
/**
 * Resolve an extensionless module path to the file that actually exists on disk:
 * prefer the compiled `.js` (running from `dist/`), otherwise the `.ts` source
 * (running from `src/` under Vite/Vitest).
 *
 * This replaces the old `__testMode()`-driven `.ts`/`.js` guess, which broke
 * whenever the assumed mode didn't match the run directory (e.g. testMode +
 * dist). Detecting from the filesystem needs no flag and is correct in every
 * mode.
 */
const withModuleExt = (base: string): string => {
  if (base.endsWith(".js") || base.endsWith(".ts")) return base;
  return fs.existsSync(base + ".js") ? base + ".js" : base + ".ts";
};
  ...
  async NamespacedfetchFilesFromDir(dir: string): Promise<Record<string, unknown>> {
    ...
    await Promise.all(
      Object.keys(_mods).map(async (key) => {
        let _fp = importSpecifier(withModuleExt(key));
